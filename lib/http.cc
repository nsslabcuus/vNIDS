...
static inline const unsigned char *_http_parse_version(
  ...
  if (NULL == (data = _match_string(data, end, http_header_1_1,
                                    sizeof(http_header_1_1) - 1))) {
...
int http_parse(const unsigned char *data, const unsigned char *end,
               HttpHeaders *headers) {
  if (end - data <= 10) {
...
bool bytestring_caseprefix(const_bytestring const &s1,
                           const_charptr const &s2) {
...
namespace HTTP {
ContextHTTP::ContextHTTP(HTTP_Conn *connection, HTTP_Flow *flow,
...
bool HTTP_PDU::ParseBuffer(flow_buffer_t t_flow_buffer,
                           ContextHTTP *t_context) {
  ...
  switch (val_case_index()) {
    case 1:
      // Parse "request"
      {
        ...
        t_request_parsing_complete =
            request_->ParseBuffer(t_flow_buffer, t_context);
    ...
    case 0:
      // Parse "reply"
      {
        ...
        t_reply_parsing_complete =
            reply_->ParseBuffer(t_flow_buffer, t_context);
...
bool HTTP_Request::ParseBuffer(flow_buffer_t t_flow_buffer,
                               ContextHTTP *t_context) {
  bool t_val_parsing_complete;
  t_val_parsing_complete = false;
  switch (parsing_state_) {
    ...
    case 1: {
      ...
      if (!(t_request_parsing_complete)) goto need_more_data;
    }
    ...
    case 2: {
      ...
      if (!(t_msg_parsing_complete)) goto need_more_data;
    }
...
ExpectBody expect_reply_body(int reply_status) {
  ...
  if ((reply_status >= 100 && reply_status < 200) || reply_status == 204 ||
      reply_status == 304)
...
bool HTTP_Reply::ParseBuffer(flow_buffer_t t_flow_buffer,
                             ContextHTTP *t_context) {
  bool t_val_parsing_complete;
  t_val_parsing_complete = false;
  switch (parsing_state_) {
    ...
    case 1: {
      ...
      if (!(t_reply_parsing_complete)) goto need_more_data;
    }

      // Parse "msg"
      if (!msg_) {
        msg_ =
            new HTTP_Message(expect_reply_body(reply()->status()->stat_num()));
      }
      parsing_state_ = 2;
      /* fall through */
    case 2: {
...
bool HTTP_RequestLine::ParseBuffer(flow_buffer_t t_flow_buffer,
  ...
  t_method_string_length = HTTP_TOKEN_re_001.MatchPrefix(
      t_begin_of_data, t_end_of_data - t_begin_of_data);
  if (t_method_string_length < 0) {
    throw binpac::ExceptionStringMismatch(
        "./http-protocol.pac:73", "[^()<>@,;:\\\"/\\[\\]?={} \\t]+",
        string((const char *)(t_begin_of_data), (const char *)t_end_of_data)
            .c_str());
  }
  int t_method__size;
  ...
  if (t_method_string_length < 0)
    throw binpac::ExceptionInvalidStringLength("./http-protocol.pac:73",
                                               t_method_string_length);
  method_.init(t_begin_of_data, t_method_string_length);
  // Evaluate 'let' and 'withinput' fields

  const_byteptr const t_dataptr_after_method =
      t_begin_of_data + (t_method__size);
  ...
  t_anonymous_field_004_string_length = HTTP_WS_re_002.MatchPrefix(
      t_dataptr_after_method, t_end_of_data - t_dataptr_after_method);
  if (t_anonymous_field_004_string_length < 0) {
    throw binpac::ExceptionStringMismatch(
        "./http-protocol.pac:74", "[ \\t]*",
        string((const char *)(t_dataptr_after_method),
               (const char *)t_end_of_data)
            .c_str());
  }
  ...
  const_byteptr const t_dataptr_after_anonymous_field_004 =
      t_dataptr_after_method + (t_anonymous_field_004__size);
  BINPAC_ASSERT(t_dataptr_after_anonymous_field_004 <= t_end_of_data);
  // Parse "uri"
  int t_uri_string_length;
  t_uri_string_length = HTTP_URI_re_003.MatchPrefix(
      t_dataptr_after_anonymous_field_004,
      t_end_of_data - t_dataptr_after_anonymous_field_004);
  if (t_uri_string_length < 0) {
    throw binpac::ExceptionStringMismatch(
        "./http-protocol.pac:75", "[/[:alnum:][:punct:]]+",
        string((const char *)(t_dataptr_after_anonymous_field_004),
               (const char *)t_end_of_data)
            .c_str());
  }
  int t_uri__size;
  ...
  // check for negative sizes
  if (t_uri_string_length < 0)
    throw binpac::ExceptionInvalidStringLength("./http-protocol.pac:75",
                                               t_uri_string_length);
  uri_.init(t_dataptr_after_anonymous_field_004, t_uri_string_length);
  // Evaluate 'let' and 'withinput' fields

  const_byteptr const t_dataptr_after_uri =
      t_dataptr_after_anonymous_field_004 + (t_uri__size);
  ...
  t_anonymous_field_005_string_length = HTTP_WS_re_002.MatchPrefix(
      t_dataptr_after_uri, t_end_of_data - t_dataptr_after_uri);
  if (t_anonymous_field_005_string_length < 0) {
    throw binpac::ExceptionStringMismatch(
        "./http-protocol.pac:76", "[ \\t]*",
        string((const char *)(t_dataptr_after_uri), (const char *)t_end_of_data)
            .c_str());
  }
  ...
  const_byteptr const t_dataptr_after_anonymous_field_005 =
      t_dataptr_after_uri + (t_anonymous_field_005__size);
  BINPAC_ASSERT(t_dataptr_after_anonymous_field_005 <= t_end_of_data);
  // Parse "version"
  version_ = new HTTP_Version();
  int t_version__size;
  t_version__size =
      version_->Parse(t_dataptr_after_anonymous_field_005, t_end_of_data);
  // Evaluate 'let' and 'withinput' fields
  ...
  if (t_val_parsing_complete) {
    // Evaluate 'let' and 'withinput' fields
    process_request_ =
        t_context->flow()->porcess_http_requestline(method(), uri(), version());
  }
  buffering_state_ = 0;
  return t_val_parsing_complete;
}
...
bool HTTP_ReplyLine::ParseBuffer(flow_buffer_t t_flow_buffer,
  ...
  const_byteptr const t_dataptr_after_version =
      t_begin_of_data + (t_version__size);
  ...
  t_anonymous_field_006_string_length = HTTP_WS_re_002.MatchPrefix(
      t_dataptr_after_version, t_end_of_data - t_dataptr_after_version);
  if (t_anonymous_field_006_string_length < 0) {
    throw binpac::ExceptionStringMismatch(
        "./http-protocol.pac:93", "[ \\t]*",
        string((const char *)(t_dataptr_after_version),
               (const char *)t_end_of_data)
            .c_str());
  }
  ...
  const_byteptr const t_dataptr_after_anonymous_field_006 =
      t_dataptr_after_version + (t_anonymous_field_006__size);
  BINPAC_ASSERT(t_dataptr_after_anonymous_field_006 <= t_end_of_data);
  // Parse "status"
  status_ = new HTTP_Status();
  int t_status__size;
  t_status__size =
      status_->Parse(t_dataptr_after_anonymous_field_006, t_end_of_data);
  // Evaluate 'let' and 'withinput' fields

  const_byteptr const t_dataptr_after_status =
      t_dataptr_after_anonymous_field_006 + (t_status__size);
  ...
  t_anonymous_field_007_string_length = HTTP_WS_re_002.MatchPrefix(
      t_dataptr_after_status, t_end_of_data - t_dataptr_after_status);
  if (t_anonymous_field_007_string_length < 0) {
    throw binpac::ExceptionStringMismatch(
        "./http-protocol.pac:95", "[ \\t]*",
        string((const char *)(t_dataptr_after_status),
               (const char *)t_end_of_data)
            .c_str());
  }
  ...
  const_byteptr const t_dataptr_after_anonymous_field_007 =
      t_dataptr_after_status + (t_anonymous_field_007__size);
  BINPAC_ASSERT(t_dataptr_after_anonymous_field_007 <= t_end_of_data);
  // Parse "reason"
  int t_reason_string_length;
  t_reason_string_length =
  ...
  if (t_reason_string_length < 0)
    throw binpac::ExceptionInvalidStringLength("./http-protocol.pac:96",
                                               t_reason_string_length);
  ...
  if (t_val_parsing_complete) {
    // Evaluate 'let' and 'withinput' fields
    process_reply_ = t_context->flow()->porcess_http_replyline(
        version(), status()->stat_num(), reason());
  }
  buffering_state_ = 0;
  return t_val_parsing_complete;
}
...
HTTP_Status::HTTP_Status() { stat_num_ = 0; }

HTTP_Status::~HTTP_Status() { stat_str_.free(); }

int HTTP_Status::Parse(const_byteptr const t_begin_of_data,
                       const_byteptr const t_end_of_data) {
  // Parse "stat_str"
  int t_stat_str_string_length;
  t_stat_str_string_length = HTTP_Status_re_008.MatchPrefix(
      t_begin_of_data, t_end_of_data - t_begin_of_data);
  if (t_stat_str_string_length < 0) {
    throw binpac::ExceptionStringMismatch(
        "./http-protocol.pac:101", "[0-9]{3}",
        string((const char *)(t_begin_of_data), (const char *)t_end_of_data)
            .c_str());
  }
  ...
  // check for negative sizes
  if (t_stat_str_string_length < 0)
    throw binpac::ExceptionInvalidStringLength("./http-protocol.pac:101",
                                               t_stat_str_string_length);
  stat_str_.init(t_begin_of_data, t_stat_str_string_length);
  // Evaluate 'let' and 'withinput' fields

  int t_HTTP_Status__size;
  const_byteptr const t_dataptr_after_stat_str =
      t_begin_of_data + (t_stat_str__size);
...
HTTP_Version::HTTP_Version() { vers_num_ = 0; }

HTTP_Version::~HTTP_Version() { vers_str_.free(); }

int HTTP_Version::Parse(const_byteptr const t_begin_of_data,
                        const_byteptr const t_end_of_data) {
  // Checking out-of-bound for "HTTP_Version:anonymous_field_009"
  if (t_begin_of_data + (5) > t_end_of_data ||
      t_begin_of_data + (5) < t_begin_of_data) {
  ...
  t_vers_str_string_length = HTTP_Version_re_010.MatchPrefix(
      (t_begin_of_data + 5), t_end_of_data - (t_begin_of_data + 5));
  if (t_vers_str_string_length < 0) {
    throw binpac::ExceptionStringMismatch(
        "./http-protocol.pac:108", "[0-9]+\\.[0-9]+",
        string((const char *)((t_begin_of_data + 5)),
               (const char *)t_end_of_data)
            .c_str());
  }
  ...
  // check for negative sizes
  if (t_vers_str_string_length < 0)
    throw binpac::ExceptionInvalidStringLength("./http-protocol.pac:108",
                                               t_vers_str_string_length);
  vers_str_.init((t_begin_of_data + 5), t_vers_str_string_length);
  // Evaluate 'let' and 'withinput' fields

  int t_HTTP_Version__size;
  const_byteptr const t_dataptr_after_vers_str =
      (t_begin_of_data + 5) + (t_vers_str__size);
...
bool HTTP_Headers::ParseBuffer(flow_buffer_t t_flow_buffer,
  ...
  for (; /* forever */; ++val__elem__it_) {
    ...
    while (!t_val__elem_parsing_complete && t_flow_buffer->ready()) {
      ...
      t_val__elem_parsing_complete =
          val__elem_->ParseBuffer(t_flow_buffer, t_context);
      if (t_val__elem_parsing_complete) {
        // Evaluate 'let' and 'withinput' fields
      }
      buffering_state_ = 0;
    }
    if (!t_val__elem_parsing_complete) goto need_more_data;
...
bool HTTP_Message::ParseBuffer(flow_buffer_t t_flow_buffer,
                               ContextHTTP *t_context) {
  bool t_val_parsing_complete;
  t_val_parsing_complete = false;
  switch (parsing_state_) {
    ...
    case 1: {
      ...
      t_headers_parsing_complete =
          headers_->ParseBuffer(t_flow_buffer, t_context);
      if (t_headers_parsing_complete) {
        // Evaluate 'let' and 'withinput' fields
      }
      if (!(t_headers_parsing_complete)) goto need_more_data;
    }
    ...
    case 2: {
      ...
      switch (body_or_not_case_index()) {
        ...
        default:
          // Parse "body"
          {
            ...
            t_body_parsing_complete =
                body_->ParseBuffer(t_flow_buffer, t_context);
      ...
      if (!(t_body_or_not_parsing_complete)) goto need_more_data;
    }
...
bool HTTP_Header::ParseBuffer(flow_buffer_t t_flow_buffer,
  ...
  t_name_string_length = HTTP_HEADER_NAME_re_011.MatchPrefix(
      t_begin_of_data, t_end_of_data - t_begin_of_data);
  if (t_name_string_length < 0) {
    throw binpac::ExceptionStringMismatch(
        "./http-protocol.pac:132", "([^: \\t]+:)",
        string((const char *)(t_begin_of_data), (const char *)t_end_of_data)
            .c_str());
  }
  int t_name__size;
  ...
  // check for negative sizes
  if (t_name_string_length < 0)
    throw binpac::ExceptionInvalidStringLength("./http-protocol.pac:132",
  ...
  t_anonymous_field_012_string_length = HTTP_WS_re_002.MatchPrefix(
      t_dataptr_after_name, t_end_of_data - t_dataptr_after_name);
  if (t_anonymous_field_012_string_length < 0) {
    throw binpac::ExceptionStringMismatch(
        "./http-protocol.pac:133", "[ \\t]*",
        string((const char *)(t_dataptr_after_name),
               (const char *)t_end_of_data)
            .c_str());
  }
  ...
  const_byteptr const t_dataptr_after_anonymous_field_012 =
      t_dataptr_after_name + (t_anonymous_field_012__size);
  BINPAC_ASSERT(t_dataptr_after_anonymous_field_012 <= t_end_of_data);
  // Parse "value"
  int t_value_string_length;
  t_value_string_length =
      (t_end_of_data) - (t_dataptr_after_anonymous_field_012);
  int t_value__size;
  t_value__size = t_value_string_length;
  UNUSED(t_value__size);
  // check for negative sizes
  if (t_value_string_length < 0)
    throw binpac::ExceptionInvalidStringLength("./http-protocol.pac:134",
                                               t_value_string_length);
...
bool MIME_Line::ParseBuffer(flow_buffer_t t_flow_buffer) {
  ...
  if (t_line_string_length < 0)
    throw binpac::ExceptionInvalidStringLength("./http-protocol.pac:138",
                                               t_line_string_length);
...
bool MIME_Lines::ParseBuffer(flow_buffer_t t_flow_buffer,
                             ContextHTTP *t_context) {
  bool t_val_parsing_complete;
  t_val_parsing_complete = false;
  if (val__elem__it_ < 0) {
...
bool HTTP_Body::ParseBuffer(flow_buffer_t t_flow_buffer,
                            ContextHTTP *t_context) {
  bool t_val_parsing_complete;
  t_val_parsing_complete = false;
  ...
  switch (val_case_index()) {
            ...
            case 2:
              ...
              if (t_flow_buffer->ready()) {
                ...
                if (t_body_string_length < 0)
                  throw binpac::ExceptionInvalidStringLength(
                      "./http-protocol.pac:148", t_body_string_length);
    ...
    case 2:
      // Parse "chunks"
      {
        ...
        t_chunks_parsing_complete =
            chunks_->ParseBuffer(t_flow_buffer, t_context);
    ...
    case 3:
      // Parse "multipart"
      {
        ...
        t_multipart_parsing_complete =
            multipart_->ParseBuffer(t_flow_buffer, t_context);
...
bool HTTP_UnknownBody::ParseBuffer(flow_buffer_t t_flow_buffer) {
  ...
  switch (val_case_index()) {
    ...
    case 0:
      // Parse "rest"
      {
        ...
        while (!t_rest_parsing_complete && t_flow_buffer->ready()) {
          ...
          if (t_rest_string_length < 0)
            throw binpac::ExceptionInvalidStringLength(
                "./http-protocol.pac:157", t_rest_string_length);
      ...
      break;
    default:
      throw binpac::ExceptionInvalidCaseIndex("HTTP_UnknownBody",
                                              val_case_index());
      break;
  }
  if (t_val_parsing_complete) {
  ...
  switch (parsing_state_) {
    ...
    case 1: {
      ...
      for (; /* forever */; ++chunks__elem__it_) {
        ...
        t_chunks__elem_parsing_complete =
            chunks__elem_->ParseBuffer(t_flow_buffer);
        if (t_chunks__elem_parsing_complete) {
          // Evaluate 'let' and 'withinput' fields
        }
        if (!t_chunks__elem_parsing_complete) goto need_more_data;
    ...
    end_of_chunks:;
      ...
      if (!(t_chunks_parsing_complete)) goto need_more_data;
    }
...
bool HTTP_Chunk::ParseBuffer(flow_buffer_t t_flow_buffer) {
  ...
  switch (parsing_state_) {
    ...
    case 1: {
      ...
      while (!t_length_line_parsing_complete && t_flow_buffer->ready()) {
        ...
        if (t_length_line_string_length < 0)
          throw binpac::ExceptionInvalidStringLength(
              "./http-protocol.pac:166", t_length_line_string_length);
      ...
      if (!(t_length_line_parsing_complete)) goto need_more_data;
    }
    ...
    case 2: {
          ...
          case 2:
            ...
            if (t_flow_buffer->ready()) {
              ...
              if (t_data_string_length < 0)
                throw binpac::ExceptionInvalidStringLength(
                    "./http-protocol.pac:167", t_data_string_length);
      ...
      if (!(t_data_parsing_complete)) goto need_more_data;
    }
    ...
    case 3: {
          ...
          // Parse "crlf"
          {
            ...
            while (!t_crlf_parsing_complete && t_flow_buffer->ready()) {
              ...
              if (t_crlf_string_length < 0)
                throw binpac::ExceptionInvalidStringLength(
                    "./http-protocol.pac:170", t_crlf_string_length);
      ...
      if (!(t_opt_crlf_parsing_complete)) goto need_more_data;
    }
...
void HTTP_Flow::NewData(const_byteptr t_begin_of_data,
...
void HTTP_Flow::NewGap(int gap_length) { flow_buffer_->NewGap(gap_length); }
void HTTP_Flow::FlowEOF() {
  flow_buffer_->set_eof();
  NewData(0, 0);
}
int HTTP_Flow::content_length() { return content_length_; }

DeliveryMode HTTP_Flow::delivery_mode() { return delivery_mode_; }

const_bytestring HTTP_Flow::end_of_multipart() { return end_of_multipart_; }

bool HTTP_Flow::porcess_http_requestline(const_bytestring const &method,
                                         const_bytestring const &uri,
                                         HTTP_Version *vers) {
  UNUSED(method);
  UNUSED(uri);
  UNUSED(vers);
...
bool HTTP_Flow::porcess_http_replyline(HTTP_Version *vers, int code,
                                       const_bytestring const &reason) {
  UNUSED(vers);
  UNUSED(code);
  UNUSED(reason);
  return true;
...
bytestring HTTP_Flow::extract_boundary(const_bytestring const &value) {
  ...
  const char *boundary_begin =
      strcasestr((const char *)value.begin(), boundary_prefix);

  if (!boundary_begin) return bytestring();

  boundary_begin += 9;

  const char *boundary_end = strcasestr(boundary_begin, ";");
  if (!boundary_end) boundary_end = (const char *)value.end();

  return bytestring((const uint8 *)boundary_begin, (const uint8 *)boundary_end);
}

bool HTTP_Flow::is_end_of_multipart(const_bytestring const &line) {
  if (line.length() < 4 + end_of_multipart_.length()) return false;

  int len = end_of_multipart_.length();

  // line =?= "--" end_of_multipart_ "--"
  return (line[0] == '-' && line[1] == '-' && line[len + 2] == '-' &&
          line[len + 3] == '-' &&
          strncmp((const char *)line.begin() + 2,
                  (const char *)end_of_multipart_.begin(), len) == 0);
}

bool HTTP_Flow::porcess_http_header(const_bytestring const &name_colon,
                                    const_bytestring const &value) {
  const_bytestring name(name_colon.begin(), name_colon.length() > 0
                                                ? name_colon.end() - 1
                                                : name_colon.end());
  ...
  if (bytestring_casecmp(name, "CONTENT-LENGTH") == 0) {
    ...
    if (bytestring_caseprefix(value, "CHUNKED")) delivery_mode_ = CHUNKED;
  }
    ...
    if (bytestring_caseprefix(value, "MULTIPART")) {
      ...
      if (end_of_multipart_.length() > 0) delivery_mode_ = MULTIPART;
    }
  }

  String name_val = String((const unsigned char *)name.begin(), name.length());
  String value_val =
      String((const unsigned char *)value.begin(), value.length());

...
from utils.wildcard_utils import extract_wildcard_mask_match_string
import os
...
class NuSMV(object):
    ...
    def generate_nusmv_trans(self,tf,end_ports):
        ...
        for rule in tf.rules:
            ...
            mask = extract_wildcard_mask_match_string(rule["match"])[0]
            match = extract_wildcard_mask_match_string(rule["match"])[1]
            ...
            if (rule["affected_by"]):
                for (aff_rule,aff_intersect,aff_ports) in rule["affected_by"]:
                    ...
                    affected = affected + ")"
                    aff_mask = extract_wildcard_mask_match_string(aff_intersect)[0]
                    aff_match = extract_wildcard_mask_match_string(aff_intersect)[1]
                    for i in range(num_parts):
                        s_range = i * HDR_VAR_LEN
                        if (self.length > i*HDR_VAR_LEN+HDR_VAR_LEN-1):
                            e_range = i*HDR_VAR_LEN+HDR_VAR_LEN-1
            ...
            if (rule["mask"] != None and rule["rewrite"] != None):
                action_mask = rule["mask"].__str__(0)
                action_rewrite = rule["rewrite"].__str__(0)
                for i in range(num_parts):
                    s_range = i * HDR_VAR_LEN
                    if (self.length > i*HDR_VAR_LEN+HDR_VAR_LEN-1):
                        e_range = i*HDR_VAR_LEN+HDR_VAR_LEN-1

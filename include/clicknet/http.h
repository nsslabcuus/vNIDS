  ...
  HTTP_Request *request() const {
    switch (val_case_index()) {
      case 1:
        break;  // OK
      default:
        throw binpac::ExceptionInvalidCase("./http-protocol.pac:27:request",
                                           val_case_index(), "true");
        break;
    }
    return request_;
  ...
  HTTP_Reply *reply() const {
    switch (val_case_index()) {
      case 0:
        break;  // OK
      default:
        throw binpac::ExceptionInvalidCase("./http-protocol.pac:28:reply",
...
class HTTP_Headers {
 public:
  ...
  HTTP_Header *operator[](int index) const {
    BINPAC_ASSERT(val());
    return (*val())[index];
  }
...
class HTTP_Message {
 public:
  ...
  HTTP_Body *body() const { return body_; }
...
class MIME_Lines {
 public:
  ...
  int size() const { return val() ? val()->size() : 0; }
  MIME_Line *operator[](int index) const {
    BINPAC_ASSERT(val());
    return (*val())[index];
  }
  ...
  HTTP_Chunks *chunks() const {
    switch (val_case_index()) {
      case 2:
        break;  // OK
      default:
        throw binpac::ExceptionInvalidCase("./http-protocol.pac:150:chunks",
                                           val_case_index(), "CHUNKED");
        break;
    }
    return chunks_;
  }
  MIME_Lines *multipart() const {
    switch (val_case_index()) {
      case 3:
        break;  // OK
      default:
        throw binpac::ExceptionInvalidCase("./http-protocol.pac:151:multipart",
                                           val_case_index(), "MULTIPART");
        break;
    }
    return multipart_;
  }
  HTTP_UnknownBody *unknown() const { return unknown_; }
  ExpectBody expect_body() const { return expect_body_; }
...
class HTTP_Chunk {
 public:
  ...
  bytestring const &crlf() const { return crlf_; }
...
class HTTP_Flow : public binpac::FlowAnalyzer {
 public:
  ...
  bool porcess_http_requestline(const_bytestring const &method,
                                const_bytestring const &uri,
                                HTTP_Version *vers);
  bool porcess_http_replyline(HTTP_Version *vers, int code,
  ...
  bool porcess_http_header(const_bytestring const &name_colon,

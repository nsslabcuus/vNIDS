...
inline uint64 pac_swap(const uint64 x) {
  return x >> 56 | (x & 0xff000000000000) >> 40 | (x & 0xff0000000000) >> 24 |
...
#define FixByteOrder(byteorder, x) \
  (byteorder == HOST_BYTEORDER ? (x) : pac_swap(x))
...
inline T UnMarshall(const u_char *data, int byteorder) {
  ...
  for (int i = 0; i < (int)sizeof(T); ++i)
    result =
        (result << 8) | data[byteorder == bigendian ? i : sizeof(T) - 1 - i];
  return result;
}

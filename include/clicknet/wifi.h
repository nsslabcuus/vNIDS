...
#define WIFI_BEACON_INTERVAL(beacon) (beacon[8] + (beacon[9] << 8))
#define WIFI_BEACON_CAPABILITY(beacon) (beacon[10] + (beacon[11] << 8))
...
#define WIFI_QOS_HAS_SEQ(wh)                                        \
  (((wh)->i_fc[0] & (WIFI_FC0_TYPE_MASK | WIFI_FC0_SUBTYPE_QOS)) == \
...
#define WIFI_AUTH_ALGORITHM(auth) (auth[0] + (auth[1] << 8))
#define WIFI_AUTH_TRANSACTION(auth) (auth[2] + (auth[3] << 8))
#define WIFI_AUTH_STATUS(auth) (auth[4] + (auth[5] << 8))

...
uint16_t click_in_cksum_pseudohdr_raw(uint32_t csum, uint32_t src, uint32_t dst,
                                      int proto, int packet_len);
#else
...
#endif
uint16_t click_in_cksum_pseudohdr_hard(uint32_t csum,
                                       const struct click_ip *iph,
                                       int packet_len);
void click_update_zero_in_cksum_hard(uint16_t *csum, const unsigned char *addr,
                                     int len);
...
static inline uint16_t click_in_cksum_pseudohdr(uint32_t data_csum,
                                                const struct click_ip *iph,
                                                int transport_len) {
  if (iph->ip_hl == 5)
    return click_in_cksum_pseudohdr_raw(data_csum, iph->ip_src.s_addr,
                                        iph->ip_dst.s_addr, iph->ip_p,
                                        transport_len);
  else
...
static inline void click_update_in_cksum(uint16_t *csum, uint16_t old_hw,
                                         uint16_t new_hw) {
 ...
 * correctly. */
static inline void click_update_zero_in_cksum(uint16_t *csum,
                                              const unsigned char *x, int len) {
  if (*csum == 0) click_update_zero_in_cksum_hard(csum, x, len);
}
